"""compat"""
import sys

PY2 = sys.version_info[0] == 2
_identity = lambda x: x


if not PY2:
    text_type = str  # pylint: disable=invalid-name
    string_types = (str,)
    integer_types = (int,)

    iterkeys = lambda d: iter(d.keys())
    itervalues = lambda d: iter(d.values())
    iteritems = lambda d: iter(d.items())

    from inspect import getfullargspec as getargspec  # pylint: disable=unused-import
    from io import StringIO  # pylint: disable=unused-import

    def reraise(tp, value, tb=None):  # pylint: disable=invalid-name,unused-argument
        """reraise"""
        if value.__traceback__ is not tb:
            raise value.with_traceback(tb)
        raise value

    implements_to_string = _identity

else:
    text_type = unicode  # pylint: disable=undefined-variable
    string_types = (str, unicode)  # pylint: disable=undefined-variable
    integer_types = (int, long)  # pylint: disable=undefined-variable

    iterkeys = lambda d: d.iterkeys()
    itervalues = lambda d: d.itervalues()
    iteritems = lambda d: d.iteritems()

    from inspect import getargspec  # pylint: disable=unused-import,ungrouped-imports
    from cStringIO import StringIO  # pylint: disable=import-error

    exec('def reraise(tp, value, tb=None):\n raise tp, value, tb')  # pylint: disable=exec-used

    def implements_to_string(cls):
        """implements to string"""
        cls.__unicode__ = cls.__str__
        cls.__str__ = lambda x: x.__unicode__().encode('utf-8')
        return cls

# Certain versions of pypy have a bug where clearing the exception stack
# breaks the __exit__ function in a very peculiar way.  The second level of
# exception blocks is necessary because pypy seems to forget to check if an
# exception happened until the next bytecode instruction?
#
# Relevant PyPy bugfix commit:
# https://bitbucket.org/pypy/pypy/commits/77ecf91c635a287e88e60d8ddb0f4e9df4003301
# According to ronan on #pypy IRC, it is released in PyPy2 2.3 and later
# versions.
#
# Ubuntu 14.04 has PyPy 2.2.1, which does exhibit this bug.
BROKEN_PYPY_CTXMGR_EXIT = False
if hasattr(sys, 'pypy_version_info'):
    class _Mgr(object):
        def __enter__(self):
            return self

        def __exit__(self, *args):
            if hasattr(sys, 'exc_clear'):
                # Python 3 (PyPy3) doesn't have exc_clear
                sys.exc_clear()  # pylint: disable=no-member
    try:
        try:
            with _Mgr():
                raise AssertionError()
        except:  # pylint: disable=try-except-raise
            raise
    except TypeError:
        BROKEN_PYPY_CTXMGR_EXIT = True
    except AssertionError:
        pass
